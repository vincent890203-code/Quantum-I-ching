...
def main() -> None:
    ...
    with st.sidebar:
        ...
        user_ticker = st.text_input(
            "股票代號 / Ticker Symbol（台股可直接輸入 2330、2317 等）",
            value="2330",
            max_chars=20,
        ).strip().upper()
        ...
        st.markdown("---")
        st.caption(
            "說明：系統會根據近期股價結構（目前以台灣股市與美股為主）生成卦象，"
    ...
    if consult:
        ...
        # 將使用者輸入轉換為實際查詢用代碼：
        # - 純數字 -> 視為台股，補上 .TW（例如 2330 -> 2330.TW）
        # - 其他（已含 . 或 -） -> 視為完整代碼，直接使用
        ...
        with st.spinner("Analyzing Market Structure & Consulting Spirits..."):
            ...
            ai_answer = oracle.ask(backend_ticker, question or "Should I buy now?")
